#!/usr/bin/env bash
# =============================================================================
# Local GRPO Baseline 训练脚本（4 卡 notebook 单机版）
#
# 用法:
#   ./run_notebook_grpo.sh                    # 默认配置
#   ./run_notebook_grpo.sh my_exp_suffix      # 指定实验名后缀
#
# 后端调用 nebula_scripts/grpo/grpo_sciknoweval_parametric.sh
# =============================================================================
set -eo pipefail
...
export SUBJECT="${SUBJECT:-biology}"     # 仅用于本地数据 fallback
...
export JOB_NAME="${JOB_NAME:-LOCAL-GRPO-${DATASET}-bs${TRAIN_BATCH_SIZE}-n${ROLLOUT_N}-lr${LR}-${SUFFIX}}"
...
echo "========================================================================"
echo "Local GRPO launch (notebook, 4×H20-3e):"
...
bash "${PROJECT_ROOT}/nebula_scripts/grpo/grpo_sciknoweval_parametric.sh"
