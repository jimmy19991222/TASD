#!/bin/bash
# =============================================================================
# DAPO Baseline 实验 - Nebula 批量提交脚本
#
# 算法：DAPO（Dynamic Advantage Policy Optimization）
# 特点：off-policy, clip-higher, dynamic sampling, entropy_coeff=0.001
# 参考：DAPO 论文 + dapo.yaml 配置
#
# 使用方式：
...
# =============================================================================
# 实验矩阵（DAPO off-policy 配置）
# =============================================================================

declare -a EXPERIMENTS=(
    # mini_batch_size | 实验标签
    "8|dapo_offpolicy_mbs8"
)
...
echo "============================================"
echo "DAPO Baseline 实验提交"
echo "============================================"
echo "实验数量: ${#EXPERIMENTS[@]}"
echo "  - DAPO: off-policy, mini_batch=8, clip-higher, entropy=0.001"
echo "数据集: ${DATASET}"
echo "模型: Qwen3-8B"
...
for EXP in "${EXPERIMENTS[@]}"; do
    ...
    CMD="nebulactl run mdl \
        ...
        --user_params=\"DATASET=$DATASET ALGORITHM=dapo MODEL_PATH=$MODEL_PATH LR=$LR SEED=$SEED TRAIN_BATCH_SIZE=$TRAIN_BATCH_SIZE GEN_BATCH_SIZE=$GEN_BATCH_SIZE MINI_BATCH_SIZE=$MINI_BATCH_SIZE ROLLOUT_N=$ROLLOUT_N JOB_NAME=$JOB_NAME PROJECT_NAME=$PROJECT_NAME GIT_BRANCH=$GIT_BRANCH GIT_COMMIT=$GIT_COMMIT\""
