#!/usr/bin/env bash
# =============================================================================
# Token-level DPO 实验批量提交
#
# 实验矩阵：6 数据集 × 2 组（reference-free / with-reference）= 12 个实验
#   Group A: TOKEN_DPO_USE_REF=False  (reference-free DPO, 仅 student logprob)
#   Group B: TOKEN_DPO_USE_REF=True   (with-reference DPO, teacher logprob 作 reference)
#
# 数据集:
#   sciknoweval/biology, sciknoweval/chemistry, sciknoweval/material,
#   sciknoweval/physics, tooluse, math500
#
# 使用方式：
#   bash nebula_scripts/submit_token_dpo.sh [--dry-run]
#
# 环境变量（需提前设置）：
