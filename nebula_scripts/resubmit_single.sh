#!/bin/bash
# =============================================================================
# 单个实验重提脚本：精确复现某次失败的实验
# 使用方式：bash nebula_scripts/resubmit_single.sh [--dry-run]
# =============================================================================
...
ADV_STD_FLOOR="none"
CLIP_RATIO_HIGH="0.2"
INCLUDE_SUCCESSFUL_ROLLOUTS="False"
...
JOB_NAME="TASD-${DATASET_SHORT}-rt_${REWARD_TYPE}-noGate-rep${REPETITION_PENALTY}-normStd-clipH${CLIP_RATIO_HIGH}-clipAdv${CLIP_ADV_VALUE}-ema${TEACHER_UPDATE_RATE}-ec${ENTROPY_COEFF}-${MODEL_SHORT}-${CURRENT_TIME}"

if [ "$DRY_RUN" = true ]; then
    echo "Job: ${JOB_NAME}"
    echo "  DATASET=$DATASET REWARD_TYPE=$REWARD_TYPE ENTROPY_GATE=$ENTROPY_GATE"
    echo "  CLIP_ADV=$CLIP_ADV CLIP_ADV_VALUE=$CLIP_ADV_VALUE"
    echo "  CLIP_RATIO_HIGH=$CLIP_RATIO_HIGH NORM_ADV_BY_STD=$NORM_ADV_BY_STD ADV_STD_FLOOR=$ADV_STD_FLOOR"
    echo "  INCLUDE_SUCCESSFUL_ROLLOUTS=$INCLUDE_SUCCESSFUL_ROLLOUTS TEACHER_UPDATE_RATE=$TEACHER_UPDATE_RATE"
    exit 0
fi
