#!/usr/bin/env bash
# =============================================================================
# RLSD Baseline 参数化训练脚本（供 Nebula sweep / 本地 notebook 调用）
#
# RLSD = arXiv:2604.03128v2
#   A_t = A_seq · clip(exp(sign(A_seq) · (logp_T - logp_S)), 1±eps_w)
#
# 与 sdpo_sciknoweval_parametric.sh 同款结构，区别：
#   - --config-name rlsd
#   - 注入 algorithm.rlsd.eps_w
#   - 注入 self_distillation.teacher_regularization (ema|hard_sync) 与 rate
...
python -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="${JOB_NAME:-rlsd_sweep}" \
