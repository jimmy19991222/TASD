...
for ALPHA in "${ALPHA_LIST[@]}"; do
    ...
    STD_TAG="-std-auto"   # NORM_ADV_BY_STD=True + ADV_STD_FLOOR=auto 固定
    EMA_TAG="-ema${TEACHER_UPDATE_RATE}"

    CURRENT_TIME=$(date +%Y%m%d_%H%M%S)
    JOB_NAME="TASD-hybrid-${DATASET_SHORT}-lr${LR}-alpha${ALPHA_TAG}${STD_TAG}-ema${TEACHER_UPDATE_RATE}-rep${REPETITION_PENALTY}-${MODEL_SHORT}-${CURRENT_TIME}"
