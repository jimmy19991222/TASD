...
for EXP in "${EXPERIMENTS[@]}"; do
    ...
    CMD="nebulactl run mdl \
        ...
        --user_params=\"DATASET=$DATASET ALGORITHM=self_teacher MODEL_PATH=$MODEL_PATH LR=$LR SEED=$SEED TRAIN_BATCH_SIZE=$TRAIN_BATCH_SIZE GEN_BATCH_SIZE=$GEN_BATCH_SIZE ROLLOUT_N=$ROLLOUT_N BETA=$BETA EMA_ALPHA=$EMA_ALPHA CLIP_VALUE=$CLIP_VALUE JOB_NAME=$JOB_NAME PROJECT_NAME=$PROJECT_NAME GIT_BRANCH=$GIT_BRANCH GIT_COMMIT=$GIT_COMMIT\""
