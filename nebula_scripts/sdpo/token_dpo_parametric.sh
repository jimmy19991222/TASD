#!/usr/bin/env bash
# =============================================================================
# Token-level DPO 参数化训练脚本（供 Nebula sweep 调用）
# 所有超参通过 nebulactl --env 注入
# 使用 baseline_token_dpo 配置（DPO-only 模式，无 JSD loss）
# =============================================================================
set +xo pipefail
...
save_path="${OSS_ROOT}/rl_models/${JOB_NAME:-token_dpo_sweep}"
...
python -m verl.trainer.main_ppo \
    --config-name baseline_token_dpo \
    seed=${SEED} \
    ...
    actor_rollout_ref.actor.self_distillation.distillation_topk=10 \
    ...
    trainer.project_name="${PROJECT_NAME:-TokenDPO}" \
    trainer.experiment_name="${JOB_NAME:-token_dpo_sweep}" \
    trainer.group_name="${GROUP_NAME:-TokenDPO}" \
