...
### 论文核心 claim（**2026-05-16 19:00 升级：TGDI → TCCA**）

> **Token-level Causal Credit Assignment (TCCA)**：对失败 sample 在 top-K 个高 divergence 位置分别让 teacher 改写，得到 K 个独立的 ΔR_{t_k} 作为 **per-token causal credit**。这是真正的 **token-level credit assignment 创新**，不是简单数据增强。

**为什么从 TGDI 升级到 TCCA**：
- ❌ 旧 TGDI 本质 = 数据增强 (Mode B append) + 微弱 seq-level advantage tweak (λ·ΔR)
- ❌ token-level credit 还是借用 GRPO uniform 或 RLSD reweight，**没有 token 级别的真创新**
- ✅ TCCA = 每个失败样本在 top-K 位置做 intervention → per-token ΔR_{t_k} → 真 token 级 causal credit
- ✅ 与 RLSD (统计学 log-ratio) / TIP (启发式 entropy×divergence) / Prior-Shift (相关性 Bayesian surprise) 形成清晰差异化

**核心 2 个 idea (修正后)**：
- 🎯 **主推**：TCCA = K 点 intervention + per-token ΔR_{t_k} credit (causal, dense, base-agnostic)
- 🔬 **Ablation**：Prior-Shift (correlational) 反衬 causal 价值
