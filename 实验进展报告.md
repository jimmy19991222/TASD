...
> **ΔR (causal counterfactual credit) 是 base-agnostic 因果层，叠加在任意 RL baseline (GRPO/RLSD) 之上能稳定提升 val_acc 且避免 SDPO/PS 的 collapse 风险。**
...
│
├── Tier 1: Prior-Shift (correlational)
│   A_t = A_seq · g_t / mean_t(g_t)
│   ❌ v1 length collapse, v2 仅 ~0.55 接近 GRPO baseline
│
├── Tier 2: Posterior-Shift (correlational, hindsight)
│   ⏸ 暂缓
│
└── Tier 3: Intervention-Credit (causal counterfactual) ← 论文 main
    A_seq = base_seq_advantage + λ · ΔR · 𝟙[intervention sample]
    A_t   = A_seq · base_token_reweight · length_scale
    支持 base ∈ {grpo, rlsd, prior_shift}
```

### 1.2 与既有 baseline 的差异
...
| **TGDI (ours)** | env reward (sparse) | **真实 ΔR** + base reweight | 无 | **是** |
...
|---|---|---|---|---|
| 🔴 P0 | Phase 2 smoke 诊断 | enable_int=True, base=grpo, stderr 抓取 | local 4-GPU 30min | 决定 Nebula 2 走向 |
| 🟢 P0 | **RLSD baseline 完整 250-step** | enable_int=False, base=rlsd | Nebula slot 1 ~7h | 填论文最大对照空缺 |
| 🟡 P1 | TGDI-v3-grpo-dR (论文 main A) | enable_int=True, base=grpo, t*=aexEOS | Nebula slot 2 (smoke 通过) | 论文 headline |
| ⚪ P1 | GRPO biology 第 3 seed | enable_int=False, base=grpo | Nebula slot 2 (smoke 失败) | 填方差分析 |
| 🟡 P2 | TGDI-v3-rlsd-dR (论文 main B) | enable_int=True, base=rlsd | future | RLSD baseline 出来后 |
| ⚪ P2 | TIP-style Q3 t* filtering | divergence_metric=q3_argmax | future, ~30 行新代码 | 强化论文 t* 选择 |
| ⚪ P2 | Epistemic token 监控指标 | 新 metric | future, ~20 行 | 提前预警 collapse |
| ⚪ P2 | TGDI 横向扩展 | chemistry/physics + biology 多 seed | future | 多 dataset main figure |
| ⏸ P3 | Tier 2 Posterior-Shift | reward-conditioned teacher fwd | future | 暂缓 |
