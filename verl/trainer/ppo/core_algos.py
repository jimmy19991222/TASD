...
def compute_tg_grpo_token_weights(
    delta_abs: torch.Tensor,
    response_mask: torch.Tensor,
    config,
) -> tuple[torch.Tensor, dict[str, float]]:
    """Compute TG-GRPO per-token weights w_t = f(|δ_t|).

    |δ_t| = |logπ_T+(y_t|s_t) - logπ_T-(y_t|s_t)| is the CDM dual-teacher
    log-prob differential, computed by the actor under no_grad. The output
    w_t is detached and s_t-measurable (PG unbiasedness preserved).

    Modes (selected via ``config.policy_loss.tg_w_mode``):
        - ``topk_traj`` (recommended): per-trajectory top-k selection — picks
          ceil(top_k * num_active) tokens with largest |δ_t| within each
          response (no length bias). Hard 0/1 weights.
        - ``topk_batch``: global top-k across the whole batch (length-biased,
          long traces dominate). Hard 0/1 weights.
        - ``sigmoid``: w_t = sigmoid(β · (|δ_t| - median_active(|δ|))).
          Soft weights in (0, 1).
        - ``hard``: w_t = 1{|δ_t| >= τ}. Hard 0/1 weights.

    All modes multiply through by response_mask so padding tokens get w_t=0.
    """
    assert delta_abs.shape == response_mask.shape, (
        f"delta_abs shape {delta_abs.shape} must match response_mask {response_mask.shape}"
    )
    pl_cfg = config.policy_loss
    mode = pl_cfg.get("tg_w_mode", "topk_traj")
    top_k = float(pl_cfg.get("tg_top_k", 0.30))
    beta = float(pl_cfg.get("tg_w_sigmoid_beta", 5.0))
    tau = float(pl_cfg.get("tg_w_hard_threshold", 0.3))

    delta_abs = delta_abs.detach()
    response_mask_f = response_mask.float()
    masked_delta = delta_abs * response_mask_f

    if mode == "topk_traj":
        bsz, T = delta_abs.shape
        w_t = torch.zeros_like(delta_abs)
        # Per-row top-k by |δ_t| over active (response_mask=1) tokens.
        for i in range(bsz):
            n_active = int(response_mask_f[i].sum().item())
            if n_active <= 0:
                continue
            k = max(1, int(round(top_k * n_active)))
            k = min(k, n_active)
            row = masked_delta[i]
            # Push pad slots to -inf so they never enter top-k.
            row_for_topk = row.masked_fill(response_mask_f[i] == 0, float("-inf"))
            _, idx = torch.topk(row_for_topk, k=k, largest=True)
            w_t[i, idx] = 1.0
        w_t = w_t * response_mask_f
    elif mode == "topk_batch":
        flat = masked_delta.view(-1)
        flat_mask = response_mask_f.view(-1)
        n_active = int(flat_mask.sum().item())
        w_t = torch.zeros_like(flat)
        if n_active > 0:
            k = max(1, int(round(top_k * n_active)))
            k = min(k, n_active)
            flat_for_topk = flat.masked_fill(flat_mask == 0, float("-inf"))
            _, idx = torch.topk(flat_for_topk, k=k, largest=True)
            w_t[idx] = 1.0
        w_t = (w_t * flat_mask).view_as(delta_abs)
    elif mode == "sigmoid":
        # Median over active tokens; falls back to mean if no active tokens.
        active_vals = masked_delta[response_mask_f.bool()]
        if active_vals.numel() > 0:
            median = torch.median(active_vals)
        else:
            median = torch.zeros((), device=delta_abs.device, dtype=delta_abs.dtype)
        w_t = torch.sigmoid(beta * (masked_delta - median)) * response_mask_f
    elif mode == "hard":
        w_t = (masked_delta >= tau).to(masked_delta.dtype) * response_mask_f
    else:  # pragma: no cover — guarded by config validation
        raise ValueError(f"Unknown tg_w_mode={mode!r}")

    w_t = w_t.detach()
    # Diagnostics over active tokens.
    n_active_total = response_mask_f.sum().clamp_min(1.0)
    selected_frac = (w_t > 0).float().sum() / n_active_total
    w_metrics = {
        "tg_grpo/w_mean": (w_t * response_mask_f).sum().item() / n_active_total.item(),
        "tg_grpo/w_max": w_t.max().item(),
        "tg_grpo/selected_token_fraction": selected_frac.item(),
        "tg_grpo/delta_abs_mean": (masked_delta.sum() / n_active_total).item(),
        "tg_grpo/delta_abs_max": masked_delta.max().item(),
    }
    return w_t, w_metrics


def compute_policy_loss_tg_grpo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """TG-GRPO policy loss: vanilla PPO clip with response_mask replaced by
    w_t · response_mask, where w_t = f(|δ_t|) is the per-token weight from
    CDM dual-teacher. Advantages A_t^GRPO are unchanged (R - R̄_group).

        loss = -Σ_t w_t · clip(πθ/π_old, 1±ε) · A_t^GRPO

    PG unbiasedness: w_t is detached and s_t-measurable (computed under
    no_grad from teacher forward and not a function of πθ).
    """
    assert config is not None
    w_t, w_metrics = compute_tg_grpo_token_weights(
        delta_abs=delta_abs, response_mask=response_mask, config=config,
    )
    # Combined token weights for the loss aggregation. response_mask is
    # already factored into w_t; we keep response_mask explicitly here for
    # clipfrac/ppo_kl which should average over original active tokens, not
    # the w_t-weighted subset.
    combined_mask = w_t  # already zero on padding
    ...
    pg_loss = agg_loss(
        loss_mat=pg_losses, loss_mask=combined_mask, loss_agg_mode=loss_agg_mode,
