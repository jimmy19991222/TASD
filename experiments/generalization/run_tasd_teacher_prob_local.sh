                                    ...
                                    for DISTILL_TOPK in "${DISTILL_TOPKS[@]}"; do
                                        for USE_SELF_AS_TEACHER in "${USE_SELF_AS_TEACHER_LIST[@]}"; do
                                            for INCLUDE_SUCCESSFUL_ROLLOUTS in "${INCLUDE_SUCCESSFUL_ROLLOUTS_LIST[@]}"; do
                                                ...
                                                EXP_NAME="TASD-${DATASET_NAME}-mbs${MINI_BATCH_SIZE}-train${TRAIN_BATCH_SIZE}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-rt${REWARD_TYPE}-tf${REWARD_TRANSFORM}-topk${DISTILL_TOPK}-usat${USE_SELF_AS_TEACHER}-isr${INCLUDE_SUCCESSFUL_ROLLOUTS}-${NORM_TAG}-${CLIP_TAG}-${MODEL_NAME}-$(date +%Y-%m-%d_%H-%M-%S)"
